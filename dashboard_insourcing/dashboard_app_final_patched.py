...
# ===================== UTILITÁRIOS DE CONVERSÃO (NOVOS) =====================
# Conversões numéricas robustas (formato BR: vírgula decimal/ponto milhar)
def coerce_to_numeric(series: pd.Series) -> pd.Series:
    ...
    mask_comma_only = s.str.contains(',', na=False) & ~s.str.contains(r'\.', na=False)
    s.loc[mask_comma_only] = s.loc[mask_comma_only].str.replace(',', '.', regex=False)
    return pd.to_numeric(s, errors='coerce')

# Divide num/den com conversão robusta (retorna percentual x100)
def safe_divide(num: pd.Series, den: pd.Series) -> pd.Series:
    num_f = coerce_to_numeric(num)
    den_f = coerce_to_numeric(den)
    return ((num_f / den_f).where(den_f != 0).fillna(0)) * 100

# Converte strings 'HH:MM:SS' em segundos
def convert_hhmmss_to_seconds(series: pd.Series) -> pd.Series:
...
def calcular_percentuais_dinamicos(df):
    ...
    (Campos _Pct foram removidos do banco)
    """
    calculos = [
...
def carregar_dados_global():
    ...
    try:
        df_metricas = pd.read_csv(
            DATA_DIR / 'fato_metricas_diarias.csv',
            low_memory=False
            # Se seu arquivo usar ';' e formato BR, considere:
    ...
    if 'Data' not in df_metricas.columns:
        possiveis = [c for c in df_metricas.columns if c.strip().lower() == 'data']
        if possiveis:
            df_metricas.rename(columns={possiveis[0]: 'Data'}, inplace=True)
        else:
            raise KeyError(f"Coluna 'Data' não encontrada. Colunas disponíveis: {df_metricas.columns.tolist()}")
    df_metricas['Data'] = pd.to_datetime(df_metricas['Data'], dayfirst=True, errors='coerce')
...
def agregar_kpi_corretamente(df, kpi_info, granularidade='ME'):
    ...
    if tipo == 'MEDIA':
        if coluna in df.columns:
            df[coluna] = coerce_to_numeric(df[coluna])
        ...
        if numerador and denominador and numerador in df.columns and denominador in df.columns:
            df[numerador] = coerce_to_numeric(df[numerador])
            df[denominador] = coerce_to_numeric(df[denominador])
            soma_num = df.groupby(grupo_col)[numerador].sum()
            soma_den = df.groupby(grupo_col)[denominador].sum()
            resultado = pd.DataFrame({'Data': soma_num.index, coluna: (soma_num / soma_den * 100).fillna(0).values})
        else:
            if coluna in df.columns:
                df[coluna] = coerce_to_numeric(df[coluna])
            resultado = df.groupby(grupo_col)[coluna].mean().reset_index()
        ...
        if numerador and peso and numerador in df.columns and peso in df.columns:
            df[numerador] = coerce_to_numeric(df[numerador])
            df[peso] = coerce_to_numeric(df[peso])
            soma_num = df.groupby(grupo_col)[numerador].sum()
            soma_peso = df.groupby(grupo_col)[peso].sum()
            resultado = pd.DataFrame({'Data': soma_num.index, coluna: (soma_num / soma_peso).fillna(0).values})
        else:
            if coluna in df.columns:
                df[coluna] = coerce_to_numeric(df[coluna])
            resultado = df.groupby(grupo_col)[coluna].mean().reset_index()
...
def criar_pivot_otimizado(df, kpi_coluna, dimensao, kpi_info, granularidade='ME', max_periodos=30):
    ...
    try:
        ...
        df_para_pivot[col_num] = coerce_to_numeric(df_para_pivot[col_num])
        df_para_pivot[col_den] = coerce_to_numeric(df_para_pivot[col_den])
pivot_num = df_para_pivot.pivot_table(index=dimensao, columns=periodo_col, values=col_num, aggfunc='sum').fillna(0)
        ...
        df_para_pivot[col_num] = coerce_to_numeric(df_para_pivot[col_num])
        df_para_pivot[col_peso] = coerce_to_numeric(df_para_pivot[col_peso])
pivot_num = df_para_pivot.pivot_table(index=dimensao, columns=periodo_col, values=col_num, aggfunc='sum').fillna(0)
